...
if( !function_exists('serano_get_image_alt') ){
	
	function serano_get_image_alt( $param_img_id ){
		
		if( empty( $param_img_id ) ){
			
			$alt_text = __('Image', 'serano');
		}
		else{
...
if( !function_exists('serano_get_image_metadata') ){
	
	function serano_get_image_metadata( $image_id ){

		$image_metadata     = array();
		$image_title        = esc_html__("Image Title", 'serano');
		$image_caption      = "";
		$image_desc         = esc_html__("Image Description", 'serano');
...
if( !function_exists('serano_hex2rgb') ){
	
	function serano_hex2rgb($hex) {
...
if( !function_exists('serano_hex2rgba') ){
	
	function serano_hex2rgba($hex, $opacity = '0') {

		$rgbval = serano_hex2rgb( $hex );
