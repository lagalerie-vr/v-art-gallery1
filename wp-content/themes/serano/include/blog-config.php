...
// pagination
if( !function_exists('serano_pagination') ){

	function serano_pagination( $current_query = null, $navigation_type = "blog-nav-minimal" ){
			...
			if( $navigation_type == "blog-nav-minimal" ){
				...
				?>
					<div class="blog-numbers">    
						<div class="blog-active"><?php echo wp_kses( $paged, 'serano_allowed_html' ); ?></div>        
						<div class="blog-total"><?php  echo wp_kses( $current_query->max_num_pages, 'serano_allowed_html' ); ?></div>    
					</div>    
				<?php
...
// comments
if( !function_exists('serano_comment') ){

	function serano_comment($comment, $args, $depth) {
		...
		echo '<span>';
		echo esc_html__('at', 'serano') . ' ' . get_comment_time() . ', ' . get_comment_date() . ' - ';
		comment_reply_link(array_merge( $args, array('reply_text' => esc_html__('Reply', 'serano'), 'before' => '', 'after' => '', 'add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth'])));
		echo '</span>';
		echo '</p>';
		
		echo '<div class="comment-text">';
		if ($comment->comment_approved == '0'){
			echo '<em>' . esc_html__("Your comment is awaiting moderation", 'serano') . '</em><br />';
		}
		comment_text();
...
if( !function_exists('serano_commentform_title') ){
	function serano_commentform_title( $args ) {
...
add_filter( 'comment_form_defaults', 'serano_commentform_title' );

// the caption displayed within single blog post hero pages
if( !function_exists('serano_blog_post_hero_caption') ){

	function serano_blog_post_hero_caption() {
...
if( !function_exists('serano_custom_excerpt_length') ){

	function serano_custom_excerpt_length( $length ) {

		return intval( serano_get_theme_options( 'clapat_serano_blog_excerpt_length' ) );
	}
}
add_filter('excerpt_length', 'serano_custom_excerpt_length');
